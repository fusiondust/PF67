...
Run:
  ...
  python pf67_seed_sample_protocols_procedures_014c2.py

Optional:
  python pf67_seed_sample_protocols_procedures_014c2.py --dry-run
  python pf67_seed_sample_protocols_procedures_014c2.py --no-restart
  python pf67_seed_sample_protocols_procedures_014c2.py --db app/data/pf67.db
...
ROOT = find_root()
BACKUP_DIR = ROOT / "_pf67_patch_backups" / ("seed_014c2_" + STAMP)
REPORT_PATH = ROOT / ("pf67_seed_014c2_report_" + STAMP + ".txt")
...
def choose_db(explicit: str | None) -> Path:
    ...
    for path in candidate_db_files():
        ...
        score = 0
        for t in tables:
            if t in ("templates", "template", "protocols", "protocol"):
                score += 10
            if "step" in t:
                score += 6
...
def detect(conn):
    ...
    p_table = first(set(lower), ["protocols", "protocol", "templates", "template"])
    if p_table:
        p_table = lower[p_table]
    else:
        print("ERROR: no protocol/template table. Tables:", ", ".join(tables))
        sys.exit(1)
    s_table = first(set(lower), ["protocol_steps", "protocol_step", "template_steps", "template_step", "steps"])
    if s_table:
...
def map_protocol(cols):
    ...
    return {
        "id": first(names, ["id", "template_id", "protocol_id"]),
        "title": first(names, ["title", "name", "template_name", "protocol_name"]),
        "description": first(names, ["description", "body", "content", "instructions", "notes", "summary"]),
        "category": first(names, ["category", "subject", "group_name", "type"]),
        "kind": first(names, ["kind", "record_type", "template_type", "protocol_type", "type"]),
...
def map_step(cols):
    ...
    return {
        ...
        "parent_id": first(names, ["protocol_id", "template_id", "parent_id"]),
        "title": first(names, ["title", "name", "step_title", "step_name"]),
        "instructions": first(names, ["instructions", "description", "body", "content", "notes", "html"]),
        "sequence": first(names, ["sequence", "sort_order", "position", "step_order", "order_index", "display_order"]),
        "created_at": first(names, ["created_at", "date_created"]),
        "updated_at": first(names, ["updated_at", "modified_at", "date_modified"]),
...
def main():
    ...
    print("PF67 Seed 014C2 starting")
    ...
    print("PF67 Seed 014C2 complete")
